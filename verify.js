...
function initVerify() {
  verifyStep = 0; // Reset on load
  renderVerify();
}

function renderVerify() {
  const steps = ['Business Info', 'Address', 'Upload Docs', 'Done'];
    ...
    container.innerHTML = `
      <div style="background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); padding: 14px;">
        ...
        <div style="display: flex; gap: 7px;">
          ...
          <button class="btn btn-primary" onclick="verifyStep=2; renderVerify()">Next: Documents ➔</button>
        </div>
      </div>`;
  } else if (verifyStep === 2) {
    container.innerHTML = `
      <div style="background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); padding: 14px;">
        <div style="font-size: 14px; font-weight: 700; margin-bottom: 14px;">Step 3: Upload Documents</div>
        <div style="border: 2px dashed var(--border); border-radius: var(--radius2); padding: 20px; text-align: center; cursor: pointer; margin-bottom: 10px; transition: border-color .2s;" onmouseover="this.style.borderColor='var(--accent)'" onmouseout="this.style.borderColor='var(--border)'">
          ...
          <div style="font-size: 12px; font-weight: 600;">Business Registration</div>
          <div style="font-size: 10px; color: var(--text3);">SECP / Trade License</div>
        </div>
        <div style="display: flex; gap: 7px; margin-top: 10px;">
          <button class="btn btn-ghost" onclick="verifyStep=1; renderVerify()">← Back</button>
          <button class="btn btn-primary" onclick="verifyStep=3; renderVerify()">Submit for Review ✓</button>
        </div>
      </div>`;
  } else {
    container.innerHTML = `
      <div style="background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); text-align: center; padding: 32px;">
        ...
        <div style="font-family: 'Syne', sans-serif; font-size: 20px; font-weight: 800; margin-bottom: 6px;">Application Submitted!</div>
        <div style="font-size: 13px; color: var(--text2); max-width: 380px; margin: 0 auto 16px; line-height: 1.6;">Our team will review your documents and verify your address within 2–3 business days.</div>
